"""Der Ausfuehrungsteil von phase12_minimalpaar laeuft hier wirklich durch -
...
NB = os.path.join(HIER, "..", "phase12_minimalpaar.ipynb")
...
def lauf(startwert=2):
    ...
    ns = dict(os=os, re=re, math=math, torch=mach_torch(), collections=collections,
              ...
              N_PAAR=32, N_INERT=16, N_TEST=48, MAX_NEW=8, CHUNK=8, TEMP=1.0,
...
def test_zelle_laeuft_durch(ergebnis):
    _, ns = ergebnis
    assert "PAAR_RESULTS" in ns, "der Ausfuehrungsteil ist nicht bis zum Ergebnis gekommen"


def test_paare_sind_wirklich_minimal(ergebnis):
    """Die eine Eigenschaft, an der alles haengt."""
    _, ns = ergebnis
    baue, minimal = ns["baue_paare"], ns["paar_ist_minimal"]
    MARKE, INERT, RUMPFE = ns["MARKE"], ns["INERT"], ns["RUMPFE"]
    P = baue(RUMPFE, MARKE)
    I = baue(RUMPFE, INERT)
    assert len(P) == len(RUMPFE) >= 8
    for (a, b), (ia, ib) in zip(P, I):
        assert minimal(a, b, MARKE)
        assert minimal(ia, ib, INERT)
        assert b == ib, "die beiden Familien muessen dieselbe B-Seite haben"
        assert len(a) - len(b) == len(MARKE)
    # und der Pruefer muss echte Verletzungen erkennen
    assert not minimal("| Service (Local Name) | X |", "| Service | Y |", MARKE)
    assert not minimal("| Service | X |", "| Service | X |", MARKE)
    assert not minimal("| Service (Local Name) | X |",
                       "| Service (Local Name) | X |", MARKE)
...
def test_urteilsordnung(ergebnis):
    _, ns = ergebnis
    u = ns["urteil_paar"]
    assert u("senkt", 0.001, 6, 0.001, 0.008, "TRAEGT") == "KOPFZEILE-IM-ROUTER"
    assert u("still", 0.001, 6, 0.001, 0.008, "TRAEGT") == "MESSFELD-UNEMPFINDLICH"
    assert u("senkt", 0.400, 6, 0.001, 0.008, "TRAEGT") == "KOPFZEILE-OHNE-WIRKUNG"
    assert u("senkt", 0.001, 6, 0.001, 0.200, "TRAEGT") == "AUFLOESUNG-ZU-GROB"
    assert u("senkt", 0.001, 1, 0.001, 0.008, "TRAEGT") == "ZU-WENIG-TRENNER"
    assert u("senkt", 0.001, 6, 0.400, 0.008, "TRAEGT") == "TRENNER-ZUFAELLIG"
    assert u("senkt", 0.001, 6, 0.001, 0.008, "BLIND") == "BLIND"
